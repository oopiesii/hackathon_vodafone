...
Дата оновлення: **19.09.2026 UTC**. Підстава — розгорнутий curated-реліз із `main 581d0d7` та підготовлений код нічного потоку `feat/night-ui`. **Нічний набір WP2/WP3/WP4/WP8a/S2 і додаткові виправлення безпеки ще не позначено як production.** Після релізу відповідальний за розгортання оновлює цю позначку та [STATUS.md](STATUS.md). Наявність коду, міграції чи успішного тесту не є підтвердженням розгортання.
...
## 2. Що вже розгорнуто, а що підготовлено
...
| WP2: позачерговий збір «Оновити зараз» і перебіг запиту | Підготовлено в нічній гілці. Перевірені дедуплікація, offline/deferred-стани та збереження FloodWait/Retry-After; не обіцяти нові дані одразу після натискання. |
| WP3: Network і Threads | Підготовлені UI-слоти та контракти. Регіони/вишки синтетичні, інтеграцію не підключено. Реальний Threads-збирач не працює. |
| WP4: калькулятор впливу в застосунку | Підготовлено в нічній гілці. Сценарна арифметика з джерелом і припущеннями; фактичних втрат не вимірює. Showcase залишається окремою ілюстрацією продукту. |
| WP8a: окремий `analyst`, S1 та runtime-провайдер | Реалізовано й перевірено без ключа та через локальні HTTP mocks. Не заявляти постійний production LLM-потік. Telegram `llm_allowed` у нічному потоці **не вмикався**. |
| S2: день/тиждень/місяць із доказами та fallback | Підготовлено сервіс і UI/API-контракт. Доступність конкретного зведення залежить від чинних прав, версій входу, повноти агрегатів і ролі читача. |
| Атомарні місячні агрегати без читання текстів | Підготовлена інтеграція 0030: фізичні покоління, coverage та приховування неповного підсумку. У розгорнутому 0020–0022 місячний view ще обчислюється з поточних curated-матеріалів. |
| Додаткові security-виправлення | Підготовлено ізоляцію кешу між сеансами/ролями, прив'язку shared-запитів до конкретного посилання та перевірки відкату релізу. До production-приймання не називати їх розгорнутими. |
...
5. **2:00 — явно позначена демонстрація наступного релізу.** У перевіреному preview нічної гілки відкрити Network: регіон → вишка → деталі. Сказати: «Синтетичний сценарій; інтеграцію не підключено». Калькулятор: 1 година × 10% абонентів дає приблизно **343 тис. грн** за обраною формулою, а не підтверджені втрати. Якщо preview недоступний, використати [showcase](https://hire.qpon/showcase/) як окрему ілюстрацію, без твердження, що це живі дані.
...
Після приймання нічного релізу preview у кроці 5 можна замінити production. До цього не обіцяти, що нові слоти, refresh та S2 доступні на публічній адресі.
...
Після підготовленої інтеграції **0030** актуальна S1-мітка постійного `running`-прогону також бере участь у цій ефективній curated-класифікації: потрібні завершений receipt, правильна версія/model/prompt та чинні права джерела й контексту. Фізичне збереження правил окремо не означає, що S1 лише декоративна або не впливає на відбір.
...
- Curated production acceptance у [звіті релізу](CURATED_DASHBOARD.md): 24h — 0, 7d — 33, 30d — 34 тематичні матеріали. Це контрольний знімок релізу, не обіцянка поточних чисел.
...
flowchart LR
  ...
  BUS -. підготовлено .-> AI[Analyst: S1 та S2]
  AI -.-> DB
...
Python-збирачі, processor і підготовлений analyst мають окремі процеси й ролі БД. NATS/outbox підтримують повторну доставку з дедуплікацією. Telegram-секрети шифруються; ключ моделі не потрапляє в браузер. Інший збирач можна інтегрувати через подію й узгоджений формат, але це все одно потребує реалізації адаптера, перевірки прав і тестів. Самого контейнера для нового джерела недостатньо.

Після приймання підготовленого релізу наступні кроки — оцінити відбір на ручній вибірці, узгодити runtime-модель/ліміт і джерела, підключити тестову телеметрію та перевірити зв'язок сигналів із відомими інцидентами. Автоматичні інциденти, автономні рекомендації джерел, Threads/X та чат на живих даних не заявляються готовими цим пакетом.
...
| Чи працюватиме без ключа? | Підготовлений analyst живе зі свіжим heartbeat, не викликає модель і використовує позначений fallback. У curated workflow відсутні нові мітки залишаються pending. Наявний завершений one-shot не зникає лише через відсутність runtime-ключа. |
| Чи безпечні посилання? | API обмежує роль/scope, строк і відкликання. Додатково підготовлені ізоляція кешу та захист від підміни scope пізньою cookie; стан їх production-приймання перевіряється окремо. |
