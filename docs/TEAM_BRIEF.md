...
Дата оновлення: **19.09.2026 UTC**. Повний нічний пакет розгорнуто о 22:19:12 UTC, виправлення I6 із `main 4bb17ff` — **22:41:58 UTC**. WP2/WP3/WP4/WP8a/S2 та міграції 0012, 0014–0016, 0030–0031 працюють. `analyst` healthy у **`waiting_key`**, runtime-ключ відсутній; Telegram `llm_allowed` — **0 джерел**. Поточний пакет і перевірки завершено; **роботу поставлено на паузу за прямим дорученням користувача**. Фаза 2 із двома чистими колами та I7 не завершені. Стан і відкат — у [MORNING_HANDOFF.md](MORNING_HANDOFF.md).
...
Поточні API й analyst — `night-20260919T224055Z`, pipeline/collectors — `night-20260919T221752Z`. Для відкату останнього пакета — **manifest `/var/lib/ufv/releases/night-20260919T224055Z.json`**: повертає лише API й analyst до попереднього перевіреного повного релізу. Схема та collectors не змінюються. Єдиний дозволений нічний restart collectors використано. Manifests 21:53/22:15 несумісні з неповними агрегатами після 0030; [контракт попереднього bridge](ROLLBACK_BRIDGE_REVIEW.md).
...
Після нового доручення на відновлення роботи можливі наступні кроки — оцінити відбір на ручній вибірці, узгодити runtime-модель/ліміт і джерела, підключити тестову телеметрію та перевірити зв'язок сигналів із відомими інцидентами. Автоматичні інциденти, автономні рекомендації джерел, Threads/X та чат на живих даних не заявляються готовими цим пакетом.
...
| Чи безпечні посилання? | API обмежує роль/scope, строк і відкликання. Ізоляцію кешу та захист від підміни scope пізньою cookie розгорнуто й перевірено; перевірки поточного пакета пройдено; два чисті кола фази 2 не виконані до паузи. |
