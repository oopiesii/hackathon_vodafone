...
**Розгорнуто 19.09.2026; останній пакет підтверджено 22:41:58 UTC:** API/analyst `main 4bb17ff`, images `night-20260919T224055Z`; pipeline залишається на `night-20260919T221752Z`. Міграції 0012, 0014–0016, 0030–0031 уже застосовані; нових у виправленнях немає. Analyst healthy, **`waiting_key`**, runtime-ключ відсутній; Telegram із `llm_allowed=true` — **0 джерел**. Це підтвердження сервісу, а не нового LLM-прогону. Поточний пакет і перевірки завершено; за прямим дорученням користувача роботу призупинено перед фазою 2/I7. Актуальна передача — [MORNING_HANDOFF.md](MORNING_HANDOFF.md).
...
Сервіс уже розгорнуто; наведені нижче команди описують подальше налаштування, а не додавання ключа, виконане цим релізом. Реальний runtime-ключ відсутній; Telegram-права не змінювалися. `ufv-api` та `ufv-analyst` мають тег `night-20260919T224055Z`, `ufv-pipeline` — `night-20260919T221752Z`. Єдиний дозволений нічний restart collectors використано під час попереднього повного релізу; останнє оновлення їх не перезапускало.
...
Для останнього пакета використовувати manifest, що повертає **лише API й analyst** до перевіреного повного релізу 22:17; processor/collectors залишаються поточними:
