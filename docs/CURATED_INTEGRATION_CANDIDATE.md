...
Після застосування 0031 тільки на ufv_checks повторний повний merged backend: **133 passed in 30,24 s**. API18203 зупинено, БД передано root/C для bridge acceptance. Виявлену несумісність rollback старого API з фізичним view root закриває окремим bridge API image: atomic coverage та явний 503 під час dirty generation. Цей образ має стати перевіреним попереднім образом перед повним релізом; компенсаційної зміни DB views під час відкату не планується.
