...
const joins = ` from core.mentions m join core.sources s on s.id=m.source_id join core.curated_items d on d.id=m.id
...
export function selection(params:Record<string,string>, review=false, share?:ShareScope) {
  ...
  const filters=["d.decision<>'deleted'"];
  ...
  if(decision==='visible')filters.push("d.decision in ('accepted','review')");
  else if(decision!=='all'){
    if(!['accepted','review','rejected','pending'].includes(decision))throw new HTTPException(400);
    filters.push('d.decision='+add(decision));
  }
  ...
  if(share?.topics.length)filters.push('d.topic=any('+add(share.topics)+'::text[])');
  if(params.q){if(share?.scope==='summary')throw new HTTPException(403);filters.push('m.quote ilike '+add('%'+params.q.slice(0,200)+'%'));}
  if(params.topic)filters.push('d.topic='+add(params.topic));
  if(params.kind)filters.push('m.kind='+add(params.kind));
  filters.push(...dashboardFilters(params,add));
  const effectiveJoins=['accepted','review','visible'].includes(decision)?joins.replace('core.curated_items','core.curated_visible_items'):joins;
  return {sql:effectiveJoins+' where '+filters.join(' and '),args};
}
...
export async function getFeed(params:Record<string,string>,review=false,share?:ShareScope) {
  ...
  const [total,topics,kinds,module]=await Promise.all([
    query('select count(*)::int count,max(m.fetched_at) last_fetched_at,max(d.analyzed_at) last_processed_at'+sql,args),
    query('select d.topic topic,count(*)::int count'+sql+' group by d.topic',args),
  ...
  const items=share?.scope==='summary' ? [] : await query(`select m.id,m.url source_url,m.published_at,d.topic topic,m.severity,m.quote text,d.summary,
    d.action,d.analyzed_at processed_at,m.fetched_at,m.edited_at,m.kind,d.reason,d.decision,d.brand,d.classifier,d.model,d.evidence_quote,m.context_id,m.duplicate_of,
  ...
  return {items:items.slice(0,50),total:total[0],topics,kinds,next_before:items.length>50?items[49].id:null,summary_only:share?.scope==='summary',classifier:'effective-classification',telegram_enabled:module[0].enabled};
}
...
export async function getDocument(id:string,review=false,share?:ShareScope) {
  ...
  const rows=await query('select s.workflow_id,m.raw_item_id from core.mentions m join core.sources s on s.id=m.source_id where m.id=$1',[id]);
  ...
  const fields=`select m.*,d.action,d.decision,d.brand,d.summary,d.reason,d.classifier,d.model,d.evidence_quote,m.quote text,m.url source_url,d.topic topic,d.analyzed_at processed_at,s.external_id username,s.kind source_kind,coalesce(s.title,s.external_id) channel_title,r.decision manual_decision`;
  const docs=await query(fields+sql+' and d.raw_item_id=$'+(args.length+1),[...args,rows[0].raw_item_id]);
  if(!docs.length)throw new HTTPException(404);
  const doc=docs[0];let context=null;
  if(doc.context_id){const parents=await query('select m.id,m.quote text,m.url source_url,m.kind'+sql+' and d.raw_item_id=(select raw_item_id from core.mentions where id=$'+(args.length+1)+')',[...args,doc.context_id]);context=parents[0]||null;}
