...
export async function getFeed(params:Record<string,string>,review=false,share?:ShareScope) {
  ...
  const items=share?.scope==='summary' ? [] : await query(`select m.id,m.url source_url,m.published_at,d.topic topic,m.severity,m.quote text,d.summary,
    d.analyzed_at processed_at,m.fetched_at,m.edited_at,m.kind,d.reason,d.decision,d.brand,d.classifier,d.model,d.evidence_quote,m.context_id,m.duplicate_of,
...
export async function getDocument(id:string,review=false,share?:ShareScope) {
  ...
  const fields=`select m.*,d.decision,d.brand,d.summary,d.reason,d.classifier,d.model,d.evidence_quote,m.quote text,m.url source_url,d.topic topic,d.analyzed_at processed_at,s.external_id username,s.kind source_kind,coalesce(s.title,s.external_id) channel_title,r.decision manual_decision`;
