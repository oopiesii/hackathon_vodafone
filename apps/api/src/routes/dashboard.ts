...
export async function getDashboard(params:Record<string,string>,review:boolean):Promise<DashboardResponse> {
 ...
 const result:DashboardResponse={
  ...
  brand_status:{level:brandLevel,title:{calm:'Спокійно',attention:'Увага',critical:'Критично',unknown:'Сигналів за добу немає'}[brandLevel],reason:brandLevel==='unknown'?(brandWeekCount?`За добу видимих згадок Vodafone немає. За 7 днів — ${brandWeekCount}; стан «спокійно» чи «увага» рахується лише за добу. Відсутність за добу не доводить відсутності проблем.`:'Видимих згадок Vodafone немає ні за добу, ні за 7 днів. Це стан збору, а не доказ спокою.'):(activeBrand[0]?.title||'Правила не виявили високого сигналу.'),negative_delta_pp:delta,href:brandLevel==='unknown'&&brandWeekCount?href({brand:'vodafone',decision:'accepted',window:'7d',from:iso(new Date(now-7*24*HOUR))}):href({brand:'vodafone',from:iso(new Date(now-24*HOUR))})},
