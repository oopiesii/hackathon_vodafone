...
refresh.get('/',async c=>{
 ...
 return c.json({requests:await query(`select ${fields} from core.refresh_requests where workflow_id=$1 order by requested_at desc,id desc limit 10`,[workflow])});
});
