...
import { ArrowUpRight, RefreshCw, ShieldCheck, ShieldAlert, RadioTower } from "lucide-react";
...
export function Dashboard({ me }: { me: Me | undefined }) {
  ...
  const query = useQuery({ queryKey: ["dashboard", workflow, window], queryFn: () => api<DashboardResponse>(`/dashboard?${new URLSearchParams({ workflow_id: workflow, window })}`), placeholderData: keepPreviousData, refetchInterval: 300000 });
  ...
  return <div className="dashboard">
    <PageHeader title="Сьогодні" actions={<button className="btn btn-outline" type="button" onClick={() => query.refetch()} disabled={query.isFetching}><RefreshCw size={16} aria-hidden="true" />Оновити дані</button>} />
    ...
    {data && <div className="dashboard-data" aria-busy={query.isFetching}>
      ...
      <div className="dashboard-grid">
        ...
        <Card className="span-5" title={data.aggregated ? "Динаміка за днями" : "Динаміка згадок"}><HourlyBars data={data.hourly} daily={data.aggregated} /></Card>
        <Card className="span-4" title="Реакції" actions={<Badge title={data.reactions.note}>Евристика</Badge>} footer={<span className="note">{count(data.reactions.observed_items)} матеріалів із реакціями. Реакція на допис не дорівнює ставленню до оператора.</span>}><ShareBar negative={data.reactions.negative} ironic={data.reactions.ironic} sad={data.reactions.sad} positive={data.reactions.positive} href={data.reactions.href} />{data.reaction_freshness && <details className="reaction-method"><summary>Коли й звідки ці реакції</summary><p>Агреговані лічильники Telegram; імена людей не збираємо. Збережені вимірювання: {time(data.reaction_freshness.oldest_at)} – {time(data.reaction_freshness.newest_at)}.</p><p>План: активні пости — кожні {count(data.reaction_freshness.active_seconds/60)} хв; після спаду — {count(data.reaction_freshness.cooling_seconds/60)} хв; сплячі — {count(data.reaction_freshness.sleeping_seconds/3600)} год.</p><p>{data.reaction_freshness.note}</p><p>Дашборд оновлює показники кожні 5 хвилин або кнопкою «Оновити дані».</p></details>}</Card>
        <Card className="span-4" title="Джерела згадок"><BarList data={data.sources.slice(0, 5).map(s => ({ id: s.id, label: s.title, value: s.count, href: s.href }))} /></Card>
        <Card className="span-4" title="Поширення" actions={<Badge title="Перепублікації не доводять незалежність джерел.">Збіги змісту</Badge>}>
          {data.spread.length ? <div className="spread-list">{data.spread.slice(0, 3).map(s => <Link key={s.id} to={s.href}><strong>{count(s.count)} матеріалів</strong><span>{count(s.source_count)} джерел</span><small>{s.third_repost_seconds === null ? "Третю перепублікацію не зафіксовано" : `До 3-го поширення: ${count(s.third_repost_seconds / 60)} хв`}</small></Link>)}</div> : <div className="chart-no-data">Збігів змісту у вибраному вікні немає</div>}
        </Card>
        <Card className="span-4" title="Вплив на Vodafone" actions={<Badge>Припущення</Badge>}><div className="dashboard-placeholder"><strong>Сценарій, а не виміряні втрати</strong><span>Вплив залежить від тривалості та частки абонентів. Калькулятор готується.</span></div></Card>
        <Card className="span-4" title="Стан мережі" actions={<Badge>Не підключено</Badge>}><div className="dashboard-placeholder"><RadioTower size={24} aria-hidden="true" /><strong>Телеметрія оператора</strong><span>Без внутрішнього API стан вишок невідомий.</span></div></Card>
        <Card className="span-4" title="Конкуренти"><BarList data={data.competitors.map(c => ({ id: c.brand, label: c.brand === "kyivstar" ? "Київстар" : c.brand === "lifecell" ? "lifecell" : c.brand, value: c.count, href: c.href }))} /></Card>
