...
export function Dashboard({ me }: { me: Me | undefined }) {
  ...
  return <div className="dashboard">
    ...
    {query.isError && <Alert tone="danger"><div>Не вдалося оновити дашборд. {errorText(query.error)} Збір даних при цьому не зупиняється — матеріали лишаються доступними у стрічці.</div><button className="btn btn-outline btn-sm" type="button" onClick={() => query.refetch()}>Спробувати знову</button><Link className="btn btn-outline btn-sm" to={`/feed?workflow_id=${workflow}`}>Відкрити стрічку</Link></Alert>}
