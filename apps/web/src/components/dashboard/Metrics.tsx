...
export function Metrics({ data }: { data: DashboardResponse }) {
  return <div className="dashboard-metrics">{(Object.keys(METRIC_LABELS) as (keyof typeof METRIC_LABELS)[]).map(key => {
    ...
    return <Link to={metric.href} className={`metric-tile metric-attention-${metric.attention ?? 0}`} key={key} title={`${metric.value === null ? "Невідомо" : count(metric.value)} · ${metric.note} ${metric.attention_note || ""}`}>
...
export function MetricsTable({ data }: { data: DashboardResponse }) {
  return <div className="tablewrap"><table><thead><tr><th>Показник</th><th>Значення</th><th>Методика</th><th>Ряд значень</th></tr></thead><tbody>{(Object.keys(METRIC_LABELS) as (keyof typeof METRIC_LABELS)[]).map(key => {
    const metric = data.metrics[key];
    return metric && <tr key={key}><td><Link to={metric.href}>{METRIC_LABELS[key]}</Link></td><td>{metric.value === null ? "Невідомо" : count(metric.value)} {metric.unit === "seconds" ? "с" : metric.unit === "percent" ? "%" : metric.unit}</td><td>{metric.note} {metric.attention_note}</td><td>{metric.series.length ? metric.series.map(count).join("; ") : "Немає ряду"}</td></tr>;
  })}</tbody></table></div>;
}
