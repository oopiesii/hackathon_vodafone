...
export function AiSources() {
  ...
  const allowAll = useMutation({ mutationFn: () => send<{ updated: number }>("/admin/ai/sources/allow-all", "POST", { llm_basis: bulkBasis }), onSuccess: () => { bulkDialog.current?.close(); void cache.invalidateQueries({ queryKey: ["ai-status"] }); } });
  ...
  return <>
    ...
    <Dialog ref={bulkDialog} title="Дозволити AI-обробку доступних джерел" titleId="ai-bulk-title"><form className="stack" onSubmit={e => { e.preventDefault(); allowAll.mutate(); }}>
      <p>Дозвіл отримають усі ввімкнені Telegram-джерела та RSS із підтвердженою підставою. RSS, заблоковані умовами видавця, лишаться вимкненими.</p>
      <Field label="Підстава рішення" hint="Зберігається для кожного джерела й потрапляє в журнал аудиту (щонайменше 10 символів).">
        <textarea required minLength={10} maxLength={1000} rows={3} value={bulkBasis} onChange={e => setBulkBasis(e.target.value)} />
      </Field>
      {allowAll.isError && <Alert tone="danger">{errorText(allowAll.error)}</Alert>}
      <button className="btn" disabled={allowAll.isPending || bulkBasis.trim().length < 10}>{allowAll.isPending ? "Вмикаємо…" : "Дозволити доступні"}</button>
    </form></Dialog>
  </>;
}
