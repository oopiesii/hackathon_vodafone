...
export function ShareBar({ negative, ironic=0, sad, positive, href }: { negative: number; ironic?:number; sad: number; positive: number; href: string }) {
  ...
  return <div className="reaction-chart">
    ...
    <div className="reaction-values">{parts.map(p => <Link to={href} key={p.kind} className={active === p.kind ? "reaction-value is-active" : "reaction-value"}><span><i className={`reaction-${p.kind}`} />{p.label}</span><strong>{p.value.toLocaleString("uk-UA")}</strong><small>{total ? `${Math.round(p.value / total * 100)}%` : "—"}</small></Link>)}</div>
  </div>;
}
